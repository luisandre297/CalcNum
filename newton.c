...
#define Newton(a,b,c,d,e)   Metodo2(0,a,b,Newtonf,c,d,0,e)
...
double Metodo2(double xaa, double xa, functionType *f, functionType2 *g, double erro, int maxIter, int TypeSec, FILE *pont_arq)
{
    ...
    printf("x = %lf  f(x)= %lf  f'(x)= %lf  erro = %lf  iteracao: 0\n",xa,pol(xa),devpol(xa),ErrorHandler.tdesvio);
    fprintf(pont_arq,"x = %lf  f(x)= %lf  f'(x)= %lf  erro = %lf  iteracao: 0\n",xa,pol(xa),devpol(xa),ErrorHandler.tdesvio);
    for(i=0 ; i<= maxIter ; i++)
    {
        x = (*g)(xa,xaa,f);
        ...
        printf("x = %lf  f(x)= %lf  f'(x)= %lf  erro = %lf  iteracao: %d\n",x,pol(x),devpol(x),ErrorHandler.tdesvio,i+1);
        fprintf(pont_arq,"x = %lf  f(x)= %lf  f'(x)= %lf  erro = %lf  iteracao: %d\n",x,pol(x),devpol(x),ErrorHandler.tdesvio,i+1);
        if(TypeSec == 1)
        {
            if(((*f)(xa)*(*f)(x))< 0) xaa = x;
            else xa = x;
...
int main()
{
    ...
    int ite;
    pont_arq = fopen("arquivo_res_newton.txt", "w");
    ...
    scanf("%d",&ite);
    r = Newton(xaa,polp,erro,ite,pont_arq);
